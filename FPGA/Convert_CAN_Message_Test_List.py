def format_can_messages(values):
    formatted_messages = []
    ...
    for value in values:
        ...
        message_id = int(parts[1], 16)  # Convert the message ID to hexadecimal
        data = parts[2].replace(" ", "")  # Remove spaces from the data section

        # Construct the formatted message
        formatted_message = f"{{ .id = 0x{message_id:02x}, .data = 0x{data} }}"
        formatted_messages.append(formatted_message)

    # Combine all messages into a list initialization format
    list_initialization = (
        "can_message_t message[] = { " + ", ".join(formatted_messages) + " };"
    )
...
formatted = format_can_messages(values)
print(formatted)
print(len(values))
