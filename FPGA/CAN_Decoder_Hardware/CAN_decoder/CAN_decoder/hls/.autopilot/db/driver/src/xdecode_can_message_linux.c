...
typedef struct {
    ...
    XDecode_can_message_uio_map maps[ MAX_UIO_MAPS ];
} XDecode_can_message_uio_info;

/***************** Variable Definitions **************************************/
static XDecode_can_message_uio_info uio_info;
...
static int uio_info_read_name(XDecode_can_message_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDecode_can_message_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDecode_can_message_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDecode_can_message_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDecode_can_message_Initialize(XDecode_can_message *InstancePtr, const char* InstanceName) {
	XDecode_can_message_uio_info *InfoPtr = &uio_info;
