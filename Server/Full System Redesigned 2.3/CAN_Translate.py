...
class CANTranslator:
    ...
    async def data_handler(self, data, translated_data, terminate_event):
        ...
        [message_length, can_id_str, data_string, total_messages_str, date_time_str] = message.split(',')

        arduino_time_raw = int(date_time_str)

        self.total_messages = int(total_messages_str)
    ...
    @staticmethod
    async def find_nth(haystack, needle, n): # message, ',', 1
        start = haystack.find(needle)
        while start >= 0 and n > 1:
            start = haystack.find(needle, start + len(needle))
            n -= 1 #just a method to make it run
        return start
