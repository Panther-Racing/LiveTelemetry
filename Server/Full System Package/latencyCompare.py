    ...
    with open(latency_in_file, 'r') as in_file, open(latency_out_file, 'r') as out_file, open(latency_diff_file, 'w') as diff_file:
        ...
        for message_in in in_data:
            for message_out in out_data:
                if message_in['Message'] == message_out['Message']:
                    latency_diff = message_out['Time'] - message_in['Time']
                    diff_file.write(f"Message: {message_in['Message']}, Latency Difference: {latency_diff}\n")
                    break
            else:
